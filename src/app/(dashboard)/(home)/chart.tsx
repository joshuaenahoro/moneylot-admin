...
>(
  (
    ...
    const tooltipLabel = React.useMemo(() => {
      ...
      if (labelFormatter) {
        return (
          <div className={twJoin('font-medium', labelClassName)}>
      ...
      return (
        <div className={twJoin('font-medium', labelClassName)}>{value}</div>
      );
    }, [
      label,
      labelFormatter,
    ...
    return (
      <div
        ref={ref}
        className={twJoin(
          'grid min-w-[8rem] items-start gap-4 rounded-lg border border-[#e5e7eb] bg-white p-6 text-xs shadow-lg',
          className,
        )}
      >
          ...
          {payload.map((item, index) => {
            ...
            return (
              <div
                key={item.dataKey}
                className={twJoin(
                  '[&>svg]:text-muted-foreground flex w-full items-stretch gap-2 [&>svg]:h-4 [&>svg]:w-4',
